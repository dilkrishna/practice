    ...
    <div class="block">
        ...
        <form action="" method="post" class="form-horizontal form-bordered" enctype="multipart/form-data">
                ...
                <div class="col-xs-6">
                    <select id="parent_id" name="parent_id" class="form-control" size="1">
                        ...
                        @foreach($parents as $p)
                            <option value="{{$p['id']}}" {{($p['id'] == $query['parent_id']) ? 'selected' : ''}}>{{$p['menu_title']}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-xs-6">
                    <input type="text" id="url" name="url" class="form-control" placeholder="Enter page url" value="{{$query['url']->url}}">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="col-xs-6">
                    ...
                    @if (!empty($query['header_image']))
                        <img id="preview" src="{{url('/uploads/page/' . $query['header_image'])}} "
                ...
                <div class="col-xs-6">
                    <label class="checkbox-inline" for="top_nav">
                        <input type="checkbox" id="top_nav" name="top_nav"
                               value="1" {{($query['top_nav'] == 1) ? 'checked' : ''}}>Main Navigation
                    </label>
                    <label class="checkbox-inline" for="footer_nav">
                        <input type="checkbox" id="footer_nav" name="footer_nav"
                               value="1" {{($query['footer_nav'] == 1) ? 'checked' : ''}}>Footer Navigation
                    </label>
                </div>
            </div>
            ...
            {{--<div class="form-group">--}}
               ...
               {{--<textarea id="meta_keywords" name="meta_keywords" rows="3"--}}
                         {{--class="form-control">{{ $query['meta_keywords'] }}</textarea>--}}
                {{--</div>--}}
            {{--</div>--}}
            ...
            {{--<div class="form-group">--}}
                ...
                {{--<textarea id="meta_description" name="meta_description" rows="3" class="form-control"--}}
                          {{--placeholder="Enter meta description">{{ $query['meta_description'] }}</textarea>--}}
                {{--</div>--}}
            {{--</div>--}}
            ...
            <div class="form-group">
                ...
                <div class="col-md-4">
                    <select id="status" name="status" class="form-control" size="1">
                        <option value="1" {{($query['status'] == 1) ? 'selected' : ''}}>Enable
                        </option>
